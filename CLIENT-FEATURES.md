...
**Audience:** integrators choosing APIs. Setup (Maven, keystores, endpoints) is in **`README.md`**.

**Branches:** `java-11-jakarta-full-wsdl` ships **26** facade classes (this list). `java-11-jakarta` ships **14** (subset of IHI search + core provider search/read/manage); do not assume full-wsdl-only clients exist on that branch.
...
- Published JAR does **not** include licensed WSDL/XSD; you supply the ADHA bundle at runtime (`wsdl/` + `schema/` under one root). See **`README.md`** and **`wsdls/readme.txt`**.
...
## Quick index — all facade classes (full-wsdl branch)
