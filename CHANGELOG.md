...
- **Build:** **`maven-source-plugin`** **`jar-no-fork`** avoids forking **`generate-sources`** when attaching **`-sources`**. **`maven-clean-plugin`** **`retryOnError`** and **`force`**. **`wsdls/ee4j-jaxws-lib-pom.xml`** **`maven-dependency-plugin`** **3.10.0**. **README** / **MAINTAINERS** clarify **`jakarta.*`** Maven coordinates vs **`javax.*`** packages for **`jaxws-rt`** **2.3.x**. **`.cursor/rules/java-maven-defaults.mdc`** documents Java **8** vs **11** release-line stacks. **`maven-javadoc-plugin`** **3.12.0** uses **`jar`** (no **`jar-no-fork`** goal in this plugin line).
