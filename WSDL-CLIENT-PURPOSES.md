...
| Purpose | WSDL service | What it does | Java facade | On `java-11-jakarta` / `java-8-javax` (14 facades) |
|---|---|---|---|:---:|
...
| **IHI — batch search** | `HI_ConsumerSearchIHIBatchAsync` | Submit/retrieve async batch IHI search | `ConsumerSearchIHIBatchAsyncClient` | Full-wsdl only |
| **IHI — create** | `HI_ConsumerCreateProvisionalIHI` | Create provisional IHI | `ConsumerCreateProvisionalIHIClient` | Full-wsdl only |
| **IHI — update** | `HI_ConsumerUpdateProvisionalIHI` | Update provisional IHI | `ConsumerUpdateProvisionalIHIClient` | Full-wsdl only |
| **IHI — merge** | `HI_ConsumerMergeProvisionalIHI` | Merge provisional IHIs | `ConsumerMergeProvisionalIHIClient` | Full-wsdl only |
| **IHI — resolve** | `HI_ConsumerResolveProvisionalIHI` | Resolve provisional IHI | `ConsumerResolveProvisionalIHIClient` | Full-wsdl only |
| **IHI — create** | `HI_ConsumerCreateUnverifiedIHI` | Create unverified IHI | `ConsumerCreateUnverifiedIHIClient` | Full-wsdl only |
| **IHI — create** | `HI_ConsumerCreateVerifiedIHI` | Create verified IHI | `ConsumerCreateVerifiedIHIClient` | Full-wsdl only |
| **IHI — update** | `HI_ConsumerUpdateIHI` | Update IHI details | `ConsumerUpdateIHIClient` | Full-wsdl only |
| **IHI — notify** | `HI_ConsumerNotifyDuplicateIHI` | Notify duplicate IHI | `ConsumerNotifyDuplicateIHIClient` | Full-wsdl only |
| **IHI — notify** | `HI_ConsumerNotifyReplicaIHI` | Notify replica IHI | `ConsumerNotifyReplicaIHIClient` | Full-wsdl only |
...
| **HPI-I — TDS search** | `HI_ProviderSearchTdsProviderIndividual` | Search TDS provider individual | `ProviderSearchTdsProviderIndividualClient` | Full-wsdl only |
| **HPI-I — TDS manage** | `HI_ProviderManageTdsProviderIndividual` | Manage TDS provider individual | `ProviderManageTdsProviderIndividualClient` | Full-wsdl only |
...
- `java-8-javax` may consume types from published **`hi-wsdl`** on the classpath instead of in-repo `wsimport`; the **same 14 facades** apply as `java-11-jakarta`.
- `java-11-jakarta-full-wsdl` exposes all **26** rows above via in-repo `wsimport`.
